...
namespace ID3Sharp.IO
{
	...
	public class EncodedString
	{
        ...
        private static Encoding specifiedEncoding;
		#endregion
		...
		public EncodedString( TextEncodingType encodingType, byte[] stringBytes )
		{
			...
			switch( encodingType )
			{
				case TextEncodingType.ISO_8859_1:
					...
					break;
				case TextEncodingType.Specified:
					this.SetSpecifiedBytes( stringBytes );
					break;
        ...
        public static ReadOnlyCollection<EncodedString>
			CreateStrings( Encoding encoding, byte[] stringData )
		{
            specifiedEncoding = encoding;
			return CreateStrings( TextEncodingType.Specified, stringData, stringData.Length, null );
		}
			...
			get
			{
				...
				switch( encodingType )
				{
					case TextEncodingType.ISO_8859_1:
						...
						break;
					case TextEncodingType.Specified:
						returnValue = this.ToSpecifiedString();
						break;
					case TextEncodingType.UTF_16:
		...
		private byte[] ToSpecifiedBytes()
		{
			int offset = 0;
			if ( hasEncodingTypePrepended ) { offset = 1; }

			byte[] stringBytes = PrepareBytes( TextEncodingType.Specified );
			for ( int charItr = 0; charItr < str.Length; charItr++ )
			{
				stringBytes[ charItr + offset ] = (byte) str[ charItr ];
			}
		...
		private string ToSpecifiedString()
		{
            return specifiedEncoding.GetString(
                GetStringDataOnly(ToSpecifiedBytes(), TextEncodingType.Specified)).Trim('\x0');
        }
		...
		private void SetSpecifiedBytes( byte[] value )
		{
