...
namespace ID3Sharp
{
	...
	public class ID3v1Tag : ID3Tag
	{
		...
		public static ID3v1Tag ReadTag( string filename, Encoding encoding )
		{
			using ( Stream stream = new FileStream( filename, FileMode.Open, FileAccess.Read ) )
			{
				return ReadTag( stream, encoding );
			}
		}
		...
		public static ID3v1Tag ReadTag( Stream stream, Encoding encoding )
		{
			ID3v1Tag tag = null;
			...
			if ( HasTag( stream ) )
			{
				...
				bytes = ReadFromFile( ID3v1Tag.TitleFieldLength, stream );
                tag.Title = BytesToString(bytes, encoding);

				bytes = ReadFromFile( ID3v1Tag.ArtistFieldLength, stream );
                tag.Artist = BytesToString(bytes, encoding);

				bytes = ReadFromFile( ID3v1Tag.AlbumFieldLength, stream );
                tag.Album = BytesToString(bytes, encoding);

				bytes = ReadFromFile( ID3v1Tag.YearFieldLength, stream );
                tag.Year = BytesToString(bytes, encoding);

				bytes = ReadFromFile( ID3v1Tag.ID3v1_1CommentsFieldLength, stream );
                tag.Comments = BytesToString(bytes, encoding);
		...
		/// <returns>The string equivalent to the specified
		/// array of bytes.</returns>
		protected static string BytesToString( byte[] bytes, Encoding encoding )
		{
